...
Сертификаты выпускаются автоматически при запуске через docker контейнер. 
Если приложение запускается без docker, необходимо выпустить эти сертификаты вручную
Для получения сертификатов вручную необходимо выполнить следующие команды в терминале:
```
